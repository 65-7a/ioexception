...
void setup() {
  ...
  pwm.begin(); // Initialise connection to the adafruit servo controller thingy
  pwm.setOscillatorFrequency(27*pow(10,6)); // Set the oscillator to 27MHz
  pwm.setPWMFreq(SERVO_FREQ);  // Tells the PWM board the speed to output to the servos
...
int moveto(int rad, int deg) {
  ...
  pwm.setPWM(Base_Horizontal, 0, map(deg, 0, 360, SERVOMIN, SERVOMAX)); // Spins the base of the robot to the desired angle

  float Base_Vert_Angle = acos((pow(rad, 2)+pow(Aftarm, 2) - pow(Forearm))/2*Forearm*rad)); // Gets the angle of the shoulder joint using the law of cosines.
  pwm.setPWM(Base_Vertical, 0, map(Base_Vert_Angle, 0, 360, SERVOMIN, SERVOMAX)); // Raises the shoulder to the desired angle
  ...
  pwm.setPWM(Elbow_Vertical, 0, map(Elbow_Vert_Angle, 0, 360, SERVOMIN, SERVOMAX)); // Raises the elbow to the desired angle
  return 1;
};
